...
export const ActualizarCargo = (req, res) => {
    ...
    const { idcargos, cargo, codigo_salario } = req.body;

    let verify = 'SELECT * FROM nomina_database.cargos where `idcargos`= ' + `'${idcargos}'`;

    let update = 'UPDATE nomina_database.cargos SET `cargo`= ' + `'${cargo}'` + ', `codigo_salario`= ' + `'${codigo_salario}'` + 'where `idcargos`= ' + `'${idcargos}'`;
